...
class NetworkTrainer():
    def __init__(self,
        ...
        self.lossF = load_lossFunction(config.get('loss_type','blur'), config, device)
    ...
    def train_process(self, data: dict[str, torch.Tensor], phase: str="train"):
        ...
        resize_scale = self.train_size / data_width
        ...
        levelmap = self.__net(ovl, bg, target_vis, resize_scale = resize_scale, get_levelmap=True)
        ...
        # alphamap = self.__vismodel.dilated_mask_gp[0] * self.lossF.base_alpha

        # self.lossF.compute_loss(self.__vismodel, alphamap, self.vis_spat_weight)
        self.lossF.compute_loss(self.__vismodel, levelmap)
        self.train_alphamap = levelmap
        ...
        if phase == "train":
            ...
            tmp_df['alpha_mean'] = torch.mean(levelmap,(1,2,3)).cpu().detach().numpy()
            tmp_df['alpha_std'] = torch.std(levelmap,(1,2,3)).cpu().detach().numpy()
    ...
    def train_save_image(self, epoch: int):
        ...
        low_ref = self.lossF.blend_with_soft_levels(self.lossF.ref_Gpyr, self.train_alphamap.squeeze(1))
        self.__vismodel.set_reference(low_ref)

        alphamap = self.__vismodel.dilated_mask_gp[0] * self.lossF.base_alpha
        blendimg = self.__vismodel.ref * (1-alphamap) + ovl * alphamap
        ...
        self.train_alphamap = self.train_alphamap.expand(-1,3,-1,-1)
