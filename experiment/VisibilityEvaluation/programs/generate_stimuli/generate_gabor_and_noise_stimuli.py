...
# --- [新規] 輝度・コントラスト設定 ---
# create_calibrated_gray_patches.py で出力された対応表をここに入力
# (輝度, ピクセル値) のタプルのリスト形式
LUMINANCE_PIXEL_MAP = [
    (100.0, 248), (90.0, 237), (80.0, 221), (70.0, 207), (60.0, 191),
    (50.0, 176), (40.0, 156), (30.0, 138), (20.0, 114), (10.0, 81), (0.0, 0)
]
MEAN_LUMINANCES_CDM2  = [50, 5]   # 平均輝度リスト (cd/m^2)
MICHELSON_CONTRASTS = [1.0, 0.8, 0.6, 0.4, 0.2] # コントラストリスト

# --- 保存設定 ---
OUTPUT_DIR = "./stimuli" # 保存先ベースフォルダ
...
if __name__ == "__main__":
        ...
        for mean_lum in MEAN_LUMINANCES_CDM2:
            for contrast in MICHELSON_CONTRASTS:
                for cpd in SPATIAL_FREQS_CPD:
                    ...
                    # 保存先フォルダの準備
                    gabor_dir = os.path.join(OUTPUT_DIR, "gabor", f"{distance}cm")
    ...
    for distance in BACKGROUND_DISTANCES_CM:
        ...
        for cpd in SPATIAL_FREQS_CPD:
            ...
            # D. 保存先フォルダの準備
            noise_dir = os.path.join(OUTPUT_DIR, "noise", f"{distance}cm")
