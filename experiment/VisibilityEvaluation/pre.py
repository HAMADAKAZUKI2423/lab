...
class ExperimentApp:
    ...
    def start_experiment(self):
        ...
        fg_prefixes = {os.path.basename(p)[0] for p in fg_img_paths}
        if not all(key in bg_images_by_prefix for key in fg_prefixes):
             messagebox.showerror("Error", "前景画像の接頭辞に対応する背景画像のグループが見つかりません。")
             self._reset_to_setup_ui()
             return

        # --- 3. Create the base randomized list of foreground images ---
        # Each foreground image is used twice. This list will be used for EACH brightness block.
        fg_trial_list = fg_img_paths * 2
        random.shuffle(fg_trial_list)

        # --- 4. Build the final trial list ---
        block_trials = []
        for fg_path in fg_trial_list:
            prefix = os.path.basename(fg_path)[0]
            if prefix not in bg_images_by_prefix or not bg_images_by_prefix[prefix]:
                ...
                self._reset_to_setup_ui()
                return
            bg_path = random.choice(bg_images_by_prefix[prefix])
    ...
    def finish_experiment(self):
        ...
        filename = os.path.join(RESULT_DIR, f"result_{p_id}_{now}.csv")
