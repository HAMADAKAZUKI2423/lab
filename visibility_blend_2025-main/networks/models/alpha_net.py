...
class AlphaGeneratorLight(INetwork):
    
    def __init__(self, in_channels=16, max_channels=128, num_layer = 7):
        super(AlphaGeneratorLight, self).__init__()
        #encoder
        ...
        self.backbone = AlphaNetEncoder(3, self.feature_channels)
        self.vis_backbone = AlphaNetEncoder(1, self.feature_channels)
        self.film_mlp = FilmMlp(self.feature_channels,self.feature_channels)
        self.decoder = FilmDecoder(self.feature_channels,2)
    ...
    def forward(self, x1, x2, tv):
        x1_code = self.backbone(x1)
        x2_code = self.backbone(x2)
        x_code = []
        for x1,x2 in zip(x1_code,x2_code):
            x_code.append(torch.cat([x1,x2],dim=1))
        tv = tv * 2.0-1.0
