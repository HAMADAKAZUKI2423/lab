...
if __name__ == '__main__':
    ...
    default_out_path = os.getcwd()+'/results/paper_background/'
    ...
    bg_full = load_img_torch(args.data + "imgs/exp/test/noise.png", args.device)
    fg = load_img_torch(args.data + "imgs/exp/test/stripes.png", args.device,"alpha")
    height = bg_full.shape[2]
    width = bg_full.shape[3]

    target_visibilities = [0.2,0.4,0.6]
    bg_initial_color = 0.5
    bg_contrast = [0,0.25,0.5,0.75,1.0]
    initial_alpha = []
    figsize = 3
    ...
    for vis_idx, visibility in enumerate(target_visibilities):
        ...
        for contrast_idx, contrast in enumerate(bg_contrast):
            stim = stimulus()
            stim.set_bg(bg_full * contrast + bg_initial_color * (1 - contrast))
            ...
            save_name = net_path + 'vis'+str(int(visibility*100)).zfill(3) + '_bg' +str(int(contrast*100)).zfill(3) + '_'
            ...
            data = Image.open(net_path + 'vis'+str(int(visibility*100)).zfill(3) + '_bg' +str(int(contrast*100)).zfill(3) + '_blend.png')
            data = np.asarray(data)
            ax[contrast_idx, vis_idx].xaxis.set_major_locator(plt.NullLocator())
            ax[contrast_idx, vis_idx].yaxis.set_major_locator(plt.NullLocator())
            ax[contrast_idx, vis_idx].imshow(data)
    ...
    for alpha_idx, alpha in enumerate(initial_alpha):
        ...
        for contrast_idx, contrast in enumerate(bg_contrast):
            ...
            save_name = alpha_path + 'alpha'+str(int(alpha*100)).zfill(3) + '_bg' +str(int(contrast*100)).zfill(3) + '_'
            blend_main(alpha_model_dict, stim, save_name)

            data = Image.open(alpha_path + 'alpha'+str(int(alpha*100)).zfill(3) + '_bg' +str(int(contrast*100)).zfill(3) + '_blend.png')
            data = np.asarray(data)
            ax[contrast_idx, alpha_idx].xaxis.set_major_locator(plt.NullLocator())
            ax[contrast_idx, alpha_idx].yaxis.set_major_locator(plt.NullLocator())
            ax[contrast_idx, alpha_idx].imshow(data)
