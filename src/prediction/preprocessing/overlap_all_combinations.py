...
def main():
    ...
    for fg_dist, bg_dist in DISTANCE_PAIRS:
        ...
        if not fg_files:
            print(f"警告: 処理対象の画像が見つかりませんでした。({fg_dist}cm)")
            continue

        print(f"--- 処理開始: {fg_dist}cm (FG) と {bg_dist}cm (BG) ---")
        print(f"前景画像: {len(fg_files)}枚")

        count = 0
        for fg_path in fg_files:
            fg_basename = os.path.basename(fg_path)
            bg_path = os.path.join(bg_dir, fg_basename)

            if not os.path.exists(bg_path):
                print(f"警告: 対応する背景画像が見つかりません、スキップします: {bg_path}")
                continue
            ...
            # --- ファイル名の生成と保存 ---
            out_path = os.path.join(out_dir, fg_basename)
