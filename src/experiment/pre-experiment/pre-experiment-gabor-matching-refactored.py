...
class ExperimentApp(ExperimentBaseUI, ExperimentTrialLoop):
    ...
    def setup_experiment_blocks(self):
        """実験ブロック構成を設定"""
        dom_eye = self.participant_id.get()  # 主眼を取得（簡略版）
    ...
    def run_trial(self):
        ...
        self.gabor_base = stimuli_utils.create_gabor_base(width_fg, height_fg, ppd_fg, 
                                                          self.spatial_freq, orientation=ori)
        self.noise_base = stimuli_utils.create_noise_base(width_fg, height_fg, ppd_fg, 
                                                          self.spatial_freq)
    ...
    def update_stimuli(self):
        ...
        self.canvas2.create_image(cx2, cy2 - gap_y_fg, image=self.photo_ref_fg, 
                                 anchor='center', tags="stim")
        self.canvas2.create_image(cx2, cy2 + gap_y_fg, image=self.photo_test, 
                                 anchor='center', tags="stim")
    ...
    def save_and_next(self, event=None):
        ...
        self.add_trial_result({
            ...
            "offset_x_right": right_res["offset_x"],
            "offset_y_right": right_res["offset_y"],
            "pd_left": left_res["pd_mean"],
            "offset_x_left": left_res["offset_x"],
            "offset_y_left": left_res["offset_y"]
        })
