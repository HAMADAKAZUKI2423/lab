...
MATCH_CONTRAST = 1.0
PRE_EXPERIMENT_DIR = Path(__file__).resolve().parent.parent
LAB_ROOT = PRE_EXPERIMENT_DIR.parents[2]
STIMULUS_OUTPUT_DIR = LAB_ROOT / "data" / "processed" / "images" / "pre-experiment-matching"


@lru_cache(maxsize=1)
def _load_defocus_stimuli_module():
    module_path = LAB_ROOT / "src" / "experiment" / "stimuli" / "defocus_stimuli.py"
    spec = importlib.util.spec_from_file_location("defocus_stimuli", module_path)
    if spec is None or spec.loader is None:
        raise ImportError(f"defocus_stimuli.py could not be loaded: {module_path}")
...
def _ensure_stimuli(app):
    ...
    module = _load_defocus_stimuli_module()
...
def update_defocus_view(app) -> None:
    ...
    defocus_stimuli = _ensure_stimuli(app)
