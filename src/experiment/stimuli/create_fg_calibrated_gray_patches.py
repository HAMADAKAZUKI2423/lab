...
def generate_calibrated_gray_patches(cd_m2_levels, max_cd_m2, min_cd_m2, offset=0.0, gamma=2.2, size=(400, 300), create_images=True):
    ...
    output_dir = os.path.join(lab_root, "data", "processed", "images", "pre-experiment-gabor", "fg_calibrated_gray_patches")
    if create_images:
...
if __name__ == "__main__":
    ...
    script_dir = os.path.dirname(os.path.abspath(__file__))
    lab_root = os.path.abspath(os.path.join(script_dir, "..", "..", ".."))
    output_dir = os.path.join(lab_root, "results", "tables", "pre-experiment-gabor", "fg_calibration_log")
    os.makedirs(output_dir, exist_ok=True)
