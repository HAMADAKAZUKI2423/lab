...
parser = argparse.ArgumentParser(description='Analyze defocus matching experiment results with contrast.')
...
L_fg = 35.0
L_bg = 15.0
final_df['Matched_AR_Contrast'] = final_df['Matched_Contrast'] * (L_fg / (L_fg + L_bg))
            ...
            for cond in condition_order:
                if patch_idx < len(ax.patches):
                    ...
                    if pd.notna(height) and height > 0:
                        ...
                        if not subset.empty:
                            m = subset['Matched_AR_Contrast'].mean()
                            d = subset['Matched_AR_Contrast'].std() 
