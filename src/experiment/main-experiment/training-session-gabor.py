# Training Session Program (Gabor版)
# py .\src\experiment\main-experiment\training-session-gabor.py
...
# --- トレーニング設定 ---
NUM_TRAINING_TRIALS = 10  # トレーニング試行数

# ==========================================

class TrainingSessionApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Training Session (Window 2)")
        ...
        # --- ステップ0: 距離入力画面の表示 ---
        self.setup_distance_input_ui()

    def setup_distance_input_ui(self):
        """ステップ0: 距離入力UIを構築し表示する"""
        # UI要素をまとめるためのフレーム
        self.setup_frame = tk.Frame(self.root, bg='gray', padx=20, pady=20)
    ...
    def setup_calibration_ui(self):
        ...
        # トレーニング開始ボタン
        btn = tk.Button(self.ctrl_frame, text="Start Training", command=self.start_training)
        btn.pack(pady=10)
        btn.focus_set()
        btn.bind('<Return>', lambda event: self.start_training())
    ...
    def start_training(self):
        ...
        # --- 3. Build the trial list (random trials, no repeats) ---
        trial_list = []
        ...
        for fg_path in fg_img_paths:
            ...
            for lum_key, bg_list in bgs_by_lum.items():
                if bg_list:
                    ...
                    trial_list.append({"bg_image": selected_bg, "fg_image": fg_path})

        # ランダムに10試行を選択
        if len(trial_list) < NUM_TRAINING_TRIALS:
            random.shuffle(trial_list)
        else:
            trial_list = random.sample(trial_list, NUM_TRAINING_TRIALS)
