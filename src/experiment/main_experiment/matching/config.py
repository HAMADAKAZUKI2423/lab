...
def _base_config(session_type: str, **overrides: Any) -> MatchingSessionConfig:
    result_root = (
        LAB_ROOT / "results" / "tables" / "main-experiment-matching" / session_type
    )
    figure_root = (
        LAB_ROOT / "results" / "figures" / "main-experiment-matching" / session_type
    )
    participant_dir = (
        LAB_ROOT / "data" / "processed" / "tables" / "main-experiment-matching"
    )
    values: dict[str, Any] = {
